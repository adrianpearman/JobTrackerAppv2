...
const handleFileSelect = (e) => (dispatch) => {
  ...
  if (file.type !== "text/csv") {
    dispatch({
      type: ACTIONS.CSV_APPLICATIONS_UPLOAD_VALID,
      payload: false,
    });
  }
  let id = e.target.id;
  ...
  reader.onload = (event) => {
    ...
    dispatch({
      type: ACTIONS.CSV_APPLICATIONS_UPLOAD_DATA,
      payload: content,
    });
  };
...
const handleSubmitFile = (e) => async (dispatch, getState) => {
  ...
  try {
    ...
    dispatch({
      type: ACTIONS.CSV_APPLICATIONS_UPLOAD_SUCCESSFULL,
      payload: true,
    });
    dispatch({
      type: ACTIONS.CSV_APPLICATIONS_UPLOAD_VALID,
...
const clearSubmitFile = (e) => (dispatch) => {
  document.getElementById(`${e}`).value = "";
  dispatch({
    type: ACTIONS.CSV_APPLICATIONS_UPLOAD_CLEAR_DATA,
  });
};
