...
import { fetchLocalStorage } from "../helperFunctions/localStorageFunctions";
...
const EditApplicationForm = () => {
  ...
  const [oldApplicationState, setOldApplicationState] = useState({});
  ...
  const fetchData = async (id, userID) => {
    fetchLocalStorage();
    try {
      const application = await axiosClient({
        method: "GET",
        url: "/api/application/id",
        params: {
          id: id,
      ...
      // Converting null values from db to strings
      data.interviewDate =
        data.interviewDate === null ? "" : data.interviewDate;
      data.responseDate = data.responseDate === null ? "" : data.responseDate;
      // Updating initial state
      setApplicationLink(data.link);
      setCompanyName(data.companyName);
      setDay(applicationDate.getDate());
      setDecision(parseInt(data.decision));
      setInterview(data.interview);
      setInterviewDate(data.interviewDate);
      setMonth(applicationDate.getMonth() + 1);
      setResponse(data.response);
      setResponseDate(data.responseDate);
      setSourceSite(data.sourceSite);
      setYear(applicationDate.getFullYear());
      // Setting a state value for the old state to be utilized in later when updating
      setOldApplicationState(data);
      // Showing Form
      setShowForm(true);
    } catch (error) {
      console.log(error);
  ...
  useEffect(() => {
    fetchData(params.id, context.user.uid);
  }, []);
